...
`graph TD   A[arx CLI] --> B[Decision Scoring]   B --> C[Trigger Next]   C -->|gates + checksum| D[Registry Command]   D --> E[Runner Plugin/Persona]   E --> F[Artifacts in memory-bank]   E --> G[Events/Traces]   C --> H[State Engine]   C --> I[Gate Evaluator]   I --> C   G --> J[Observability Aggregate]`
![[Untitled diagram _ Mermaid Chart-2025-09-01-034213 1.png]]
#### Runtime Data Flow

`sequenceDiagram   participant User   participant CLI as arx CLI   participant Scorer as decision_scoring/advanced_score.py   participant Trigger as tools/orchestrator/trigger_next.py   participant Gates as tools/gates/gate_evaluator.py   participant State as tools/orchestrator/state.py   participant Persona as tools/runner/plugins/*   participant MB as memory-bank/*   participant Obs as logs/*    User->>CLI: invoke command (e.g., orchestrate)   CLI->>Scorer: score candidates   Scorer-->>CLI: decision {NEXT_STEP|OPTION_SET|ASK_CLARIFY}   CLI->>Trigger: map to registry id, verify   Trigger->>Gates: evaluate gates/contexts   Gates-->>Trigger: pass/fail details   Trigger->>State: update state (idempotent)   Trigger->>Persona: execute role (dry-run by default)   Persona->>MB: emit artifacts (validated)   Persona->>Obs: append events + decision_traces   Obs-->>User: aggregated observability`

![[Untitled diagram _ Mermaid Chart-2025-09-01-034548 2.png]]
---
...
`--- description: "What this rule does" globs: ["**/*.ts"] alwaysApply: false ---`
...
`for f in (ROOT / ".cursor/rules/roles").glob("*.mdc"):     txt = f.read_text(encoding="utf-8")     m = re.search(r"^globs:\s*\[(.*?)\]\s$", txt, re.M)     ...`

`def transition(new_state: str, correlation_id: str | None = None) -> Dict[str, Any]:     """Idempotent transition. Writes only when the state changes."""     data = load_state()     cur = data.get("state")     ...`

`def evaluate_gates() -> Dict[str, Any]:     commands = _load_registry()     state = _load_state()     attach = _load_attachments()     attached_domains = _domains_attached(attach)     rules_index = _load_rules_index()     ...`
...
`DEFAULT_WEIGHTS = {"intent":0.30, ...} DEFAULT_THRESH  = {"conf_high":0.75, ...} ... result["decision"] = {"type":"NEXT_STEP"|"OPTION_SET"|"ASK_CLARIFY"|"RISK_ALERT"}`
...
`flows:   feature_request_to_pr:     guards: [branch_not_main, dry_run_unless_allowed, artifacts_present, git_clean]     nodes: {analyze_requirements, design_solution, implement_feature, code_review, create_pr}     edges: when conditions control transitions`

Flow execution engine:

`class FlowRunner:   def execute_flow(...):     _execute_flow_guards()     dag = _build_execution_dag()     results = _execute_dag()     summary = _generate_execution_summary()`

#### Orchestration Trigger

`- Loads registry - Scores candidates - Enforces gates - Verifies registry checksum - Enforces allowlist (arx, tools/run_role.py) - Executes or enqueues (optional)`

#### Mermaid: Feature Request to PR

`graph LR   AR[Analyze Requirements] --> DS[Design Solution]   DS --> IF[Implement Feature]   IF --> CR[Code Review]   CR --> PR[Create PR]`
...
`subcommands: flow, tasks, memory, obs - arx flow lint|run|render|list - arx tasks plan|print|export - arx memory index|query|stats|purge - arx obs serve --port --addr`

#### Registry Commands and Gates

`commands:   - id: planning-to-audit     requires: { states_any_of: ["PLANNING_DONE"], gates_passed_all_of: ["PLANNING_GATE"] }     contexts: { must_exist: ["memory-bank/plan/Action_Plan.md"] }`
...
`{   "required": ["schema_version","state","history","last_updated"],   "properties": {"history": {"items": {"required": ["ts","from","to"]}}} }`
...
`pip install -e . which arx && arx --help python3 tools/prestart/prestart_composite.py python3 tools/quickstart.py  # readiness → PO → planning → audit → PE`

#### Flows (safe defaults)

`arx flow lint --flow=feature_request_to_pr arx flow run --flow=feature_request_to_pr --dry-run`

#### Decision + Trigger

`python3 tools/decision_scoring/advanced_score.py python3 tools/orchestrator/trigger_next.py --dry-run --auto-candidates`
...
`AR_METRICS_PORT=9108 arx obs serve python3 tools/observability/aggregate.py`

#### Governance & Validation

`python3 tools/rules/validate.py python3 tools/schema/validate_artifacts.py python3 tools/gates/gate_evaluator.py`
...
`from tools.flow.flow_runner import FlowRunner runner = FlowRunner("flow/flow_registry.yaml") sum = runner.execute_flow("feature_request_to_pr", {"feature_name":"SLA dashboard"}, dry_run=True) print(sum)`
